...
export const checkMarkAttendence = async (req, res) => {
    try {
        const registeredUser = await User.findOne({ database: req.body.database, uuid: req.body.uuid }).maxTimeMS(30000);
        ...
        if (registeredUser) {
            ...
            res.status(200).json({ savedAttendence, message: "User logged in successfully!" });
        }
    } catch (error) {
